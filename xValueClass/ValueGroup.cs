...
namespace xLib.xValueClass
{
	...
	public class ValueGroup
	{
		...
		[SerializeField]private xNode.NodeObject.NodeString[] nodeEncrypt;
		private string KeyExtra
		{
			get
			{
				...
				{
					tempString += nodeEncrypt[i].Value;
				}
				return tempString;
			}
		}
		...
		internal virtual object SerializedObjectRaw
		{
			get
			{
				...
				if(nodeSetting.canDebug) Debug.LogFormat(nodeSetting.objDebug,nodeSetting.objDebug.name+":SerializedObject:Get:{0}:{1}",MnPlayer.CurrentId,jObject.ToString());
				return jObject;
			}
			set
			{
				...
				if(nodeSetting.canDebug) Debug.LogFormat(nodeSetting.objDebug,nodeSetting.objDebug.name+":SerializedObject:Set:{0}:{1}",MnPlayer.CurrentId,value);
				...
				if(nodeSetting.UseRpc && !MnRPC.inRpc)
				{
					if(MnPlayer.IsMy)
					{
						...
						MnPhoton.ins.RPC(nodeSetting.RpcTarget,nodeSetting.Key,stringJson);
						return;
					}
				}
				...
				if(Values != null)
				{
					if(nodeSetting.canDebug) Debug.LogFormat(nodeSetting.objDebug,nodeSetting.objDebug.name+":Values:true:{0}",MnPlayer.CurrentId);
					for (int i = 0; i < iSerializableObject.Length; i++)
					{
						ISerializableObject jsonInterface = iSerializableObject[i];
		...
		internal virtual object SerializedObjectName
		{
			get
			{
				...
				if(nodeSetting.canDebug) Debug.LogFormat(nodeSetting.objDebug,nodeSetting.objDebug.name+":SerializedObjectName:Get:{0}:{1}",MnPlayer.CurrentId,jObject.ToString());
				return jObject;
			}
		}
		#endregion
		...
		{
			if(nodeSetting.canDebug) Debug.LogFormat(nodeSetting.objDebug,nodeSetting.objDebug.name+":CallMulti:{0}",MnPlayer.CurrentId);
			MnPhoton.ins.RPC(nodeSetting.RpcTarget,nodeSetting.Key,SerializedObjectRaw.ToString());
		}
