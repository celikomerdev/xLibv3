...
namespace xLib.libAdvert.xIronSource
{
	public class AdInterstitial : AdvertBase
	{
		...
		{
			Debug.LogException(new UnityException($"{this.name}:onInterstitialAdLoadFailedEvent:{error}"),this);
			OnLoadFailBase();
		}
		...
		{
			Debug.LogException(new UnityException($"{this.name}:onInterstitialAdShowFailedEvent:{error}"),this);
			OnCloseBase();
		}
