...
namespace xLib.libAdvert.xIronSource
{
	public class AdRewarded : AdvertBase
	{
		...
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAvailabilityChangedEvent:{status}",this);
		...
		{
			Debug.LogException(new UnityException($"{name}:onRewardedVideoAdShowFailedEvent:{error}"),this);
			OnCloseBase();
		}
		...
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAdOpenedEvent",this);
			OnShowBase();
		}
		...
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAdStartedEvent",this);
		}
		
		//Invoked when the video ad finishes playing.
		private void onRewardedVideoAdEndedEvent()
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAdEndedEvent",this);
		}
		
		//Invoked when the user completed the video and should be rewarded. 
		private void onRewardedVideoAdRewardedEvent(IronSourcePlacement placement)
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAdRewardedEvent:{placement.ToString()}",this);
		...
		{
			if(CanDebug) Debug.Log($"{name}:onRewardedVideoAdClosedEvent",this);
			OnCloseBase();
		}
		#endregion
