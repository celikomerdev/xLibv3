...
namespace xLib.libAdvert.xIronSource
{
	public class AdOfferwall : AdvertBase
	{
		...
		{
			Debug.LogException(new UnityException($"{this.name}:onOfferwallShowFailedEvent:{error}"),this);
			OnCloseBase();
		}
		...
		{
			Debug.LogException(new UnityException($"{this.name}:onGetOfferwallCreditsFailedEvent:{error}"),this);
			OnRewardBase(0);
		}
